...
// Series ID format: LASST{FIPS2}00000000000003  (e.g., LASST060000000000003)
...
async function main() {
  ...
  const batches = chunk(allSeriesIds, 25);
  ...
  // Load existing JSON to merge (don’t blow away OEWS fields)
  ...
  for (const s of mergedSeries) {
    ...
    if (!row) {
      console.warn(`No LAUS values for ${abbr} (${id})`);
  ...
  console.log(`Wrote ${OUT_FILE} — filled unemployment_rate for ${filled}/${Object.keys(STATES).length} states`);
